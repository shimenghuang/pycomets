...
class PCM(Comet):
    ...
    def test(
        ...
        for ii in range(rep):
            self.pvals[ii], self.stats[ii], self.rY[:, ii], self.rT[:, ii] = _pcm_test(
                Y,
                X,
                Z,
                reg_yonxz,
...
def _pcm_test(
    ...
    if estimate_variance:
        ...
        def vhat(X, Z):
            ...
            vtemp = np.max(
                np.column_stack([reg_vonxz.predict(XZ), np.repeat(0, XZ.shape[0])]),
                axis=1,
            )
            return vtemp + chat
    ...
    L = rY * rT
    stat = np.sqrt(Yte.shape[0]) * np.mean(L) / np.sqrt(np.mean(L**2) - np.mean(L) ** 2)
    if np.isnan(stat):
        stat = -np.Inf
