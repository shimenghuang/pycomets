...
from pycomets.helper import _cov_to_cor


def _coin_test(rY, rX,
               alternative="two.sided", type="max",
    ...
    if alternative not in ["two.sided", "less", "greater"]:
        raise Warning(
            'alternative needs to be one of "quadratic" or "max".')

    if type not in ["quadratic", "max"]:
        raise Warning(
            'type needs to be one of "quadratic" or "max".')

    if distribution not in ["asymptotic", "approximate"]:
        raise Warning(
            'distribution needs to be one of "asymptotic" or "approximate".')
